...
<soap:Envelope
      ...
      objects: {
        "KotakBankAPI": {
          ...
          methods: {
            ...
            "GetMessageId": {
              displayName: "Generate new messageId",
              type: "read",
              parameters: {
                "pLeadNumber": {
                ...
                "pTypeOfRequest": {
                  displayName: "pTypeOfRequest",
                  description: "Type of request",
                  type: "string"
                }
              },
              requiredParameters: ["pLeadNumber", "pTypeOfRequest"],
              outputs: ["messageId"]
            }
          }
        }
  ...
  async function onexecuteKotakBank(methodName, properties, parameters, configuration) {
    switch (methodName) {
      ...
      case "GetMessageId":
        await kotakGetMessageId(parameters);
        break;
  ...
  function kotakGetMessageId(parameters, configuration) {
    return new Promise((resolve, reject) => {
      var leadNumber = parameters["pLeadNumber"].toString();
      var typeOfReturn = parameters["pTypeOfRequest"].toString();
      leadNumber = leadNumber.substr(0, 9).padStart(9, "0");
      typeOfReturn = typeOfReturn.substr(0, 3).padStart(3, "0");
      var name = makeUniqId(20 - 9 - 3);
      var result = `${leadNumber}${typeOfReturn}${name}`;
